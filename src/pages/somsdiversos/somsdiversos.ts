...
@Component({
  templateUrl: 'somsdiversos.html'
})
export class SomsdiversosPage {

  /* EDIT THESE */
  title: string = "Sons Diversos";
  base_url: string = "https://sound-board-cc576.firebaseapp.com/";
  sounds_url: string = "Somsdiversos";
  counter: number =1;  
  sounds: any = [];
media: any = null;
  icon: string;
      ...
      .subscribe(
        data => {
          ...
          for(let link of links) {
            ...
            let imageUrl = 'assets/lightpurple-button-unclicked.png';
            let imageUrlUnclicked = 'assets/lightpurple-button-';            
            let color = '#A13C8C';           
  ...
  play(sound) { 
   ...
   if (this.counter >= 5 && this.global.getMyGlobalNiggerPaid() == false){  
      this.appodeal.show(this.appodeal.AD_TYPES.INTERSTITIAL);
         this.counter = 0;
      console.log(this.global.getMyGlobalNiggerPaid())
    }else{
      this.counter++;
    }
